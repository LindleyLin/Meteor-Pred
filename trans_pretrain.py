...
from utils import get_loader, GradualWarmupScheduler
...
optimizer = torch.optim.AdamW(trans_model.parameters(), lr=config["lr"], weight_decay=1e-4)
...
for e in range(config["epochs"]):
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            x_0 = cnn_extractor(x_0)
            x_0 = x_0.view(B, seg_len, -1)

        c = trans_model(c, x_0)

        loss = torch.nn.functional.mse_loss(c, x_0)
        train_loss += loss.item()
        loss.backward()
        optimizer.step()
    warmUpScheduler.step()
    torch.save(trans_model.state_dict(), "model/trans_model.pth")
